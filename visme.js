...
(async () => {
  ...
  await page.type('[placeholder="Search millions of photos"]',"covid vaccine"); //Type the category of image you want
  await page.keyboard.press("Enter");

  await page.waitForSelector('[alt="white and black labeled bottle"]');
  await page.click('[alt="white and black labeled bottle"]');
  ...
  await fontSize[12].click(); //Changethe font size
  ...
  await page.waitForSelector(".graphics-item-wrapper");
  ...
  await chooseColor[8].click(); //Change the color of icon
  ...
  let newx = await page.$$(".coords-panel__col .editable-field.coords-panel__value.editable-field--center");
  await newx[2].click({ clickCount: 2 });
  await newx[2].type("1200");
 
  await prevXY[3].click();
  
  let newy = await page.$$(".editable-field__input.editable-field__coords-input.editable-field__input--center.editable-field__input--dynamic-underline-mode");
  await newy[0].click({ clickCount: 2 });
  await newy[0].type("1800");
  await page.keyboard.press("Enter");
  ...
  await page.waitForTimeout(12000); //Wait for atleast 12 seconds so that the image is downloaded
  await page.close(); //Close the Visme page and proceed to the next task
  
  //Next task is to post your design on LinkedIn
  const pageTwo = await browser.newPage();
  //Go to login page of LinkedIn
  await pageTwo.goto("https://www.linkedin.com/home");

  //Sign in and enter your login credentials
  await pageTwo.waitForSelector(".nav__button-secondary");
  await pageTwo.click(".nav__button-secondary");

  await pageTwo.waitForSelector('[aria-label="Email or Phone"]');
  await pageTwo.type('[aria-label="Email or Phone"]',"manikaursodhi@gmail.com");
  await pageTwo.waitForSelector('[aria-label="Password"]');
  await pageTwo.type('[aria-label="Password"]',"Manikaur@22");

  await pageTwo.waitForSelector('[aria-label="Sign in"]');
  await pageTwo.click('[aria-label="Sign in"]');
  await pageTwo.waitForNavigation();

  //Post the design you've created earlier
  await pageTwo.waitForSelector('[aria-label="Add a photo"]');
  await pageTwo.click('[aria-label="Add a photo"]');
  
  await pageTwo.waitForTimeout(10000); //During this time select the image from your PC (Not a part of automation)

  await pageTwo.waitForSelector(".ml2.artdeco-button.artdeco-button--2.artdeco-button--primary.ember-view");
  await pageTwo.click(".ml2.artdeco-button.artdeco-button--2.artdeco-button--primary.ember-view");

  //Add some text to explain your post
  let typeOne = await pageTwo.$$('[aria-label="Text editor for creating content"]');
  await typeOne[0].type("Hey #Connections I have created this poster using Visme website. It is important for everyone to get vaccinated. COVID 19-vaccines are effective. They can keep you from getting and spreading the virus that causes COVID-19.")
  
  await pageTwo.waitForSelector('[aria-label="Save this post"]');
  await pageTwo.click('[aria-label="Save this post"]');
